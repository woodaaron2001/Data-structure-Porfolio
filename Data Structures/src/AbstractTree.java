 ...
 */
public abstract class AbstractTree<E> implements Tree<E>,BinaryTree<E> {
    ...
    public int numChildren(Position<E> p) {
		int count = 0;
		//moves through all child position of the node p and adds them
    ...
    public int depth(Position<E> p) throws IllegalArgumentException {
    	...
    	//runs recursively to root adding one for each link between root and node
    	if(isRoot(p)) {
    		return 0;
    	}
    	else {
    ...
    public int height(Position<E> p) throws IllegalArgumentException {
        int h = 0;
        //iterate through the each child 
        //get the greatest height from both children and return it
    ...
    private void preorderSubtree(Position<E> p, List<Position<E>> snapshot) {
    	//preOrder means we visit before we recur-so add snapshot at beginning
    	snapshot.add(p);
    	if (left(p) != null)
    ...
    private void postorderSubtree(Position<E> p, List<Position<E>> snapshot) {
      ...
      if (right(p) != null)
          preorderSubtree(right(p), snapshot);
    //postOrder means we visit after we recur-so add snapshot at end
      snapshot.add(p);
    }
    ...
    public Iterable<Position<E>> breadthfirst() {
    	...
    	//breadth first is more awkward than preOrder and postOrder 
    	//Must rely on stack to iterate through the positions
    	
    	 List<Position<E>> snapshot = new ArrayList<>();
    	 //used the queue we made from previous assignments
    	 
    	 LinkedQueue<Position<E>> q = new LinkedQueue<Position<E>>();
    	 q.enqueue(root());
    	 while(!q.isEmpty()) {
    		 snapshot.add(q.first());
    		 //we move through each node left to right by adding them to the queue
    		 //each node will add their children to the back of the queue so they can be visited in order
    		 if(left(q.first()) != null) {
    			 q.enqueue(left(q.first()));
    		 }
    		 if(right(q.first()) != null) {
    			 q.enqueue(right(q.first()));
    			 
    		 }
    		 //dequeue so we can see the next value in the queue
    		 q.dequeue();
