...
public abstract class AbstractHashMap<K, V> extends AbstractMap<K, V> {
    ...
    private void resize(int newCap) {
    	//we store entries so we can use the hashFunction to put them back into the new array
    	ArrayList<Entry<K,V>> storeEntries = new ArrayList<Entry<K,V>>();
    	
    	//entry set gives us an easy way to iterate over all entrySet-add them all to storeEntries
    	for(Entry<K,V> entry : entrySet()) {
    		storeEntries.add(entry);
    	}
    	
    	//capacity set to newCap and n to 0 as n will increase in size with every put()
    	capacity = newCap;
    	n = 0;
    	
    	
    	//call createTable to recreate array with new size
    	createTable();
    	
    	//hashing back into array
    	for(Entry<K,V> entry : storeEntries) {
